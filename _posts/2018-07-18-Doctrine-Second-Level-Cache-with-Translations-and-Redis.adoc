...
== Doctrine Behaviors
This bundle was different, because we include trait in our class we do not have  *`$translations`* field, also if you open trait you will see there is no metadata for that field, it is added later in EventListener but that doesn't stop us to add that field with metadata in our class manually.
